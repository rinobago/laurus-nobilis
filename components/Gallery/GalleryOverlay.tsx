...
export default function GalleryOverlay({ open, onClose, images, initialIndex = 0 }: { open: boolean; onClose: () => void; images: Img[]; initialIndex?: number }) {
                    ...
                    <div className="flex-1 min-h-0 flex items-center justify-center px-[clamp(16px,4vw,40px)]">
                        <div className="relative w-full max-w-270">
                            ...
                            <div className="relative w-full aspect-3/2 max-h-[calc(100dvh-180px)]">
                                <Image key={current.src} src={current.src} alt={current.alt} fill={true} className="object-contain" sizes="100vw" priority={safeIndex === initialIndex} />
                            </div>
                        </div>
                    </div>
