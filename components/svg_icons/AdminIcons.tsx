export function SearchIcon({ className }: { className?: string }) {
    return (
        <svg
            className={className}
            style={{ aspectRatio: "1 / 1" }}
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <path
                d="M21.0002 21L16.6602 16.66"
                stroke="black"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M11 19C15.4183 19 19 15.4183 19 11C19 6.58172 15.4183 3 11 3C6.58172 3 3 6.58172 3 11C3 15.4183 6.58172 19 11 19Z"
                stroke="black"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
        </svg>
    );
}

export function FilterIcon({ className }: { className?: string }) {
    return (
        <svg
            className={className}
            viewBox="0 0 16 16"
            fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <path
                d="M2.63086 3H13.3703C13.4672 3 13.562 3.02814 13.6431 3.08099C13.7243 3.13384 13.7884 3.20914 13.8276 3.29772C13.8668 3.38631 13.8794 3.48437 13.8639 3.57999C13.8484 3.6756 13.8054 3.76466 13.7403 3.83634L9.63061 8.35697C9.54694 8.449 9.50058 8.56892 9.50058 8.6933V12.2324C9.50058 12.3147 9.48026 12.3958 9.44142 12.4683C9.40258 12.5409 9.34642 12.6028 9.27793 12.6484L7.27793 13.9818C7.20263 14.032 7.11512 14.0608 7.02473 14.0652C6.93433 14.0695 6.84445 14.0493 6.76466 14.0066C6.68486 13.9639 6.61816 13.9003 6.57165 13.8227C6.52515 13.745 6.50058 13.6562 6.50058 13.5657V8.6933C6.50058 8.56892 6.45422 8.449 6.37055 8.35697L2.26089 3.83634C2.19573 3.76466 2.15279 3.6756 2.1373 3.57999C2.1218 3.48437 2.13442 3.38631 2.1736 3.29772C2.21279 3.20914 2.27686 3.13384 2.35804 3.08099C2.43921 3.02814 2.53399 3 2.63086 3V3Z"
                stroke="black"
                strokeWidth="1.33012"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
        </svg>
    );
}

export function CalendarIcon({ className }: { className?: string }) {
    return (
        <svg
            className={className}
            viewBox="0 0 16 16"
            fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <path
                d="M12.6667 2.66666H3.33333C2.59695 2.66666 2 3.26361 2 3.99999V13.3333C2 14.0697 2.59695 14.6667 3.33333 14.6667H12.6667C13.403 14.6667 14 14.0697 14 13.3333V3.99999C14 3.26361 13.403 2.66666 12.6667 2.66666Z"
                stroke="black"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M10.666 1.33334V4.00001"
                stroke="black"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M2 6.66666H14"
                stroke="black"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M5.33398 1.33334V4.00001"
                stroke="black"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M11.334 9.33334H7.33398"
                stroke="black"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M8.66602 12H4.66602"
                stroke="black"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M4.66602 9.33334H4.67268"
                stroke="black"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M11.334 12H11.3407"
                stroke="black"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
        </svg>
    );
}

export function ActionDots({ className }: { className?: string }) {
    return (
        <svg
            className={className}
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <path
                d="M12 13C12.5523 13 13 12.5523 13 12C13 11.4477 12.5523 11 12 11C11.4477 11 11 11.4477 11 12C11 12.5523 11.4477 13 12 13Z"
                stroke="black"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M12 6C12.5523 6 13 5.55228 13 5C13 4.44772 12.5523 4 12 4C11.4477 4 11 4.44772 11 5C11 5.55228 11.4477 6 12 6Z"
                stroke="black"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M12 20C12.5523 20 13 19.5523 13 19C13 18.4477 12.5523 18 12 18C11.4477 18 11 18.4477 11 19C11 19.5523 11.4477 20 12 20Z"
                stroke="black"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
        </svg>
    );
}
